...
foreach(($body['teams'] ?? []) as $team){
  ...
  : ($old['match_points'] ?? null);
    ...
    $pdo->prepare(
      ...
      $p['price']          ?? 0,
      isset($p['totalPoints'])    ? (int)$p['totalPoints']    : (int)($old['total_points']    ?? 0),
      isset($p['battingPoints'])  ? (int)$p['battingPoints']  : (int)($old['batting_points']  ?? 0),
      isset($p['bowlingPoints'])  ? (int)$p['bowlingPoints']  : (int)($old['bowling_points']  ?? 0),
      isset($p['fieldingPoints']) ? (int)$p['fieldingPoints'] : (int)($old['fielding_points'] ?? 0),
      $matchPoints,
